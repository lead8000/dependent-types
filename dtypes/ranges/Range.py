...
class Range:
    ...
    def union(self, rng: Rangelike) -> Union[Rangelike, None]:
        ...
        if rng_a.isdisjoint(rng_b):
            if rng_a.end == rng_b.start and (rng_a.include_end or rng_b.include_start):
                return Range(start=rng_a.start, end=rng_b.end, include_start=rng_a.include_start, include_end=rng_b.include_end)
            return ranges.RangeSet(rng_a, rng_b)
        # merge 'em
    ...
    def __invert__(self) -> 'ranges.RangeSet':
        """
