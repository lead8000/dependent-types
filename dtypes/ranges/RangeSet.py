...
class RangeSet(Iterable):
    ...
    def add(self, rng: Rangelike) -> None:
        ...
        try:
            ...
            index = 0
            node = temp_ranges.first.value._ranges.first
            while node:
                if node.prev and node.prev.value.end == node.value.start \
                    and (node.prev.value.include_end or node.value.include_start):
                    node.prev.value.end = node.value.end
                    temp_ranges.first.value._ranges.pop(index)
                else:
                    index += 1
                node = node.next
